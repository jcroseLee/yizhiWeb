...
export default function CaseDetailPage() {
  ...
  // 用户信息悬停卡片组件
  const UserInfoPopover = ({ userId, nickname, children }: { userId: string; nickname: string; children: React.ReactNode }) => {
    const [userInfo, setUserInfo] = useState<{
      profile: { nickname: string | null; avatar_url: string | null; exp: number; reputation: number; title_level: number } | null
      stats: { publishedCases: number; likesReceived: number } | null
    } | null>(null)
    ...
    return (
      <Popover onOpenChange={(open) => {
        ...
        <PopoverTrigger asChild>
          <span className="cursor-pointer hover:text-stone-600 transition-colors">{children}</span>
        </PopoverTrigger>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {userInfo.stats && (
                      <div className="flex items-center gap-3 text-xs text-stone-400">
                        <span>案例 {userInfo.stats.publishedCases}</span>
                        <span>·</span>
                        <span>获赞 {userInfo.stats.likesReceived}</span>
                      </div>
                    )}
                  </div>
                </div>
                      ...
                      <div className="bg-white border border-stone-200 rounded-lg p-4 sm:p-5">
                        <div className="flex items-center gap-2 mb-2">
                          {data.adopted_comment?.profiles?.id && data.adopted_comment?.profiles?.nickname ? (
                            <UserInfoPopover userId={data.adopted_comment.profiles.id} nickname={data.adopted_comment.profiles.nickname}>
